...
export class AwsService {
  ...
  async uploadToS3(filename: string, file: any): Promise<string> {
    ...
    //   });
    // });
    throw new HttpErrors.NotImplemented('AWS S3 upload is disabled. Use local storage instead.');
  }
  ...
  async getSecrets(secretName: string): Promise<Secrets> {
    ...
    //   );
    // });
    throw new HttpErrors.NotImplemented('AWS Secrets Manager is disabled.');
  }
}
